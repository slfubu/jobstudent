(function () {
  ...
  window.showSection = function (sectionId) {
    // Before the protected application is loaded, only auth sections exist.
    if (!document.getElementById('appLayout')) {
      showAuthSection(sectionId);
      return;
    }
    // After app bootstrap core.js replaces this global with the full version.
    showAuthSection(sectionId);
  };
  ...
  // Expose minimal helpers because dynamically loaded legacy modules expect them.
  ...
  async function loadProtectedApplication() {
    ...
    appBootPromise = (async function () {
      ...
      // App-only third-party libraries are not sent to the Browser before login.
      for (const src of APP_DEPENDENCIES) await loadScript(src);
      if (window.pdfjsLib) {
        window.pdfjsLib.GlobalWorkerOptions.workerSrc = 'https://cdnjs.cloudflare.com/ajax/libs/pdf.js/3.11.174/pdf.worker.min.js';
      }

      // Load application code only after the protected HTML exists.
      // core.js must NOT initialize until every dependent module is available.
      ...
      // Show the protected shell, then initialize navigation/dashboard once.
      const auth = document.getElementById('authLayout');
      ...
      // Preserve the privileged-user security notice after the app has loaded.
      try {
