...
function renderTimeTable(logs) {
    try {
        ...
        // Mobile: สร้างพื้นที่รายการแบบ Card อัตโนมัติ
        // เดิม CSS ซ่อน #timeLogTable บนมือถือ แต่ไม่มี .mobile-time-list ใน HTML
        let mobileList = document.getElementById('mobileTimeLogList');
        if (!mobileList && timeLogTable) {
            mobileList = document.createElement('div');
            mobileList.id = 'mobileTimeLogList';
            mobileList.className = 'mobile-time-list';

            const tableContainer = timeLogTable.closest('.table-container');
            if (tableContainer && tableContainer.parentNode) {
                tableContainer.insertAdjacentElement('afterend', mobileList);
            } else {
                timeLogTable.insertAdjacentElement('afterend', mobileList);
            }
        }

        if (mobileList) mobileList.innerHTML = '';

        // --- [ส่วนที่ 1] คำนวณเดือนจากข้อมูลจริง ---
        let labelMonth = "";
        if (logs && logs.length > 0) {
            const firstLogDate = logs[0].date;
            const dateObj = parseDateString(firstLogDate);
            if (dateObj) {
                labelMonth = dateObj.toLocaleDateString('th-TH', { month: 'long', year: 'numeric' });
            }
        }
        ...
        // --- [ส่วนที่ 2] แสดงข้อมูลส่วนตัวสำหรับใบพิมพ์ ---
        ...
        if (!logs || logs.length === 0) {
            ...
            if (mobileList) {
                ...
                empty.style.cssText = 'text-align:center;color:#999;padding:28px 16px;background:#fff;border:1px dashed #ddd;border-radius:10px;margin-top:10px;';
                empty.textContent = 'ไม่พบประวัติการทำงานเดือนนี้';
        ...
        for (const [dateStr, dayLogs] of Object.entries(groupedLogs)) {
            ...
            const dateObjForRow = parseDateString(dateStr);
            if (dateObjForRow) {
                const d = dateObjForRow.getDate();
                const m = dateObjForRow.getMonth() + 1;
            ...
            if (mobileList) {
                ...
                daySummary.style.cssText = 'font-size:12px;font-weight:600;white-space:nowrap;';
                daySummary.textContent = `${dailyHours.toFixed(1)} ชม. • ${dailyAmount.toLocaleString()} บาท`;

                dateHeader.appendChild(dateText);
                dateHeader.appendChild(daySummary);
                mobileList.appendChild(dateHeader);
            }

            dayLogs.forEach((log, index) => {
                let inTime = String(log.checkIn || '-');
                let outTime = String(log.checkOut || '-');
                ...
                if (mobileList) {
                    ...
                    card.className = 'mobile-time-card';

                    const timeRow = document.createElement('div');
                    timeRow.className = 'mt-row';

                    const time = document.createElement('div');
                    time.className = 'mt-time';
                    time.textContent = `${inTime} - ${outTime}`;

                    const hours = document.createElement('div');
                    hours.style.cssText = 'font-size:13px;font-weight:600;color:#555;';
                    const logHours = parseFloat(log.hours || 0);
                    hours.textContent = `${Number.isFinite(logHours) ? logHours : 0} ชม.`;
                    ...
                    status.textContent = log.status ? `สถานะ: ${log.status}` : 'รายการลงเวลา';
                    ...
                    editBtn.title = 'แก้ไข';
                    editBtn.innerHTML = '<i class="material-icons" style="font-size:16px;vertical-align:middle;">edit</i> แก้ไข';
                    editBtn.addEventListener('click', () => onEditTimeLog(log));

                    const deleteBtn = document.createElement('button');
                    deleteBtn.type = 'button';
                    deleteBtn.className = 'btn btn-danger';
                    deleteBtn.title = 'ลบ';
                    deleteBtn.innerHTML = '<i class="material-icons" style="font-size:16px;vertical-align:middle;">delete</i> ลบ';
                    deleteBtn.addEventListener('click', () => onDeleteTimeLog(log.id));
        ...
        // Mobile: ยอดรวมทั้งเดือน
        if (mobileList) {
            const totalBox = document.createElement('div');
            totalBox.style.cssText = 'display:flex;justify-content:space-between;align-items:center;background:#f1f3f5;border-radius:10px;padding:14px 15px;margin:16px 0 6px;font-weight:700;';

            const totalLabel = document.createElement('span');
            totalLabel.textContent = 'รวมเดือนนี้';

            const totalValue = document.createElement('span');
            totalValue.style.color = 'var(--secondary-color)';
            totalValue.textContent = `${grandTotalHours.toFixed(1)} ชม. • ${grandTotalAmt.toLocaleString()} บาท`;

            totalBox.appendChild(totalLabel);
            totalBox.appendChild(totalValue);
            mobileList.appendChild(totalBox);
        }
